...
__device__ __forceinline__ d3 d3sub(d3 a, d3 b) { return {a.x-b.x, a.y-b.y, a.z-b.z}; }
__device__ __forceinline__ double d3dot(d3 a, d3 b) { return a.x*b.x + a.y*b.y + a.z*b.z; }
__device__ __forceinline__ d3 d3scale(d3 a, double s) { return {a.x*s, a.y*s, a.z*s}; }
__device__ __forceinline__ d3 d3add(d3 a, d3 b) { return {a.x+b.x, a.y+b.y, a.z+b.z}; }
...
__device__ inline double pt_tri_sqr_dis(d3 p, const double* __restrict__ tri, d3* cp) {
    ...
    if (d1 <= 0.0 && d2 <= 0.0) { *cp = a; return d3dot(ap, ap); }

    d3 bp = d3sub(p, b);
    double d3_ = d3dot(ab, bp), d4 = d3dot(ac, bp);
    if (d3_ >= 0.0 && d4 <= d3_) { *cp = b; d3 v = d3sub(p,b); return d3dot(v,v); }

    double vc = d1*d4 - d3_*d2;
    if (vc <= 0.0 && d1 >= 0.0 && d3_ <= 0.0) {
        double v = d1 / (d1 - d3_);
        *cp = d3add(a, d3scale(ab, v));
        d3 v2 = d3sub(p, *cp); return d3dot(v2, v2);
    }

    d3 cp2 = d3sub(p, c);
    double d5 = d3dot(ab, cp2), d6 = d3dot(ac, cp2);
    if (d6 >= 0.0 && d5 <= d6) { *cp = c; d3 v = d3sub(p,c); return d3dot(v,v); }

    double vb = d5*d2 - d1*d6;
    if (vb <= 0.0 && d2 >= 0.0 && d6 <= 0.0) {
        double w = d2 / (d2 - d6);
        *cp = d3add(a, d3scale(ac, w));
        d3 v = d3sub(p, *cp); return d3dot(v, v);
    }
